...
class DenoisingDiffusion(object):
    def __init__(self, args, config):
        ...
        # NEW: LPIPS model for perceptual distance
        self.lpips_model = lpips.LPIPS(net='vgg').to(self.device)
        self.lpips_model.eval()

        # NEW: metrics log file
        os.makedirs(self.config.data.ckpt_dir, exist_ok=True)
        self.metrics_log_path = os.path.join(self.config.data.ckpt_dir, "train_metrics.txt")
        if not os.path.exists(self.metrics_log_path):
            with open(self.metrics_log_path, "w") as f:
                f.write("step,epoch,noise_loss,scc_loss,avg_time,psnr,ssim,lpips\n")

    # ========== Metric helpers ==========

    def _to_uint8_numpy(self, x: torch.Tensor):
        """
        x: (B,3,H,W) in [0,1] on any device -> (B,H,W,3) uint8
        """
        x = x.detach().clamp(0.0, 1.0).cpu().numpy()   # (B,3,H,W)
        x = np.transpose(x, (0, 2, 3, 1))              # (B,H,W,3)
        x = (x * 255.0).round().astype(np.uint8)
        return x

    def _batch_psnr(self, pred: torch.Tensor, ref: torch.Tensor) -> float:
        pred_np = self._to_uint8_numpy(pred)
        ref_np = self._to_uint8_numpy(ref)
        vals = []
        for i in range(pred_np.shape[0]):
            vals.append(
                peak_signal_noise_ratio(ref_np[i], pred_np[i], data_range=255)
            )
        return float(np.mean(vals)) if vals else 0.0

    def _batch_ssim(self, pred: torch.Tensor, ref: torch.Tensor) -> float:
        pred_np = self._to_uint8_numpy(pred)
        ref_np = self._to_uint8_numpy(ref)
        vals = []
        for i in range(pred_np.shape[0]):
            vals.append(
                structural_similarity(ref_np[i], pred_np[i],
                                      data_range=255, channel_axis=-1)
            )
        return float(np.mean(vals)) if vals else 0.0

    @torch.no_grad()
    def _batch_lpips(self, pred: torch.Tensor, ref: torch.Tensor) -> float:
        """
        LPIPS expects [-1,1] tensors on same device as model.
        Here we compare pred vs ref (both in [0,1]).
        For speed, we limit to at most 4 images.
        """
        if pred.shape[0] > 4:
            pred = pred[:4]
            ref = ref[:4]

        pred_n = pred.detach().clamp(0.0, 1.0).to(self.device) * 2.0 - 1.0
        ref_n = ref.detach().clamp(0.0, 1.0).to(self.device) * 2.0 - 1.0

        d = self.lpips_model(pred_n, ref_n)
    ...
    def train(self, DATASET):
        ...
        for epoch in range(self.start_epoch, self.config.training.n_epochs):
            ...
            for i, (x, y) in enumerate(train_loader):
                ...
                if self.step % 10 == 0:
                    avg_time = data_time / (i + 1)

                    # compute pred_x in eval mode for metrics
                    self.model.eval()
                    with torch.no_grad():
                        out_eval = self.model(x)
                        if "pred_x" in out_eval:
                            pred_x = out_eval["pred_x"].clamp(0.0, 1.0)   # (B,3,H,W)
                            # treat first 3 channels of input as low-light reference
                            ref_x  = x[:, :3, :, :].clamp(0.0, 1.0)
                            psnr_val  = self._batch_psnr(pred_x, ref_x)
                            ssim_val  = self._batch_ssim(pred_x, ref_x)
                            lpips_val = self._batch_lpips(pred_x, ref_x)
                        else:
                            psnr_val = ssim_val = lpips_val = float("nan")
                    self.model.train()

                    print(
                        "step:{}, noise_loss:{:.5f} scc_loss:{:.5f} time:{:.5f} "
                        "psnr:{:.3f} ssim:{:.3f} lpips:{:.4f}".format(
                            self.step,
                            noise_loss.item(),
                            scc_loss.item(),
                            avg_time,
